...
class Player(Ship):
    ...
    def update(self):
        ...
        if keys[pygame.K_w]:
            self.vel += pygame.math.Vector2(0, -self.thrust_strength).rotate(-self.angle)
        if keys[pygame.K_s]:
            self.vel += pygame.math.Vector2(0, self.thrust_strength * 0.6).rotate(-self.angle)
...
def main():
    ...
    while level <= TOTAL_LEVELS:
        enemy = Enemy((SCREEN_WIDTH * 0.75, SCREEN_HEIGHT / 2), level)
        ...
        while enemy.hp > 0:
            ...
            for bullet in bullets.copy():
                if bullet.owner == player and pygame.sprite.collide_circle(bullet, enemy):
                    enemy.hp -= PLAYER_DAMAGE
                    bullet.kill()
                elif bullet.owner == enemy and pygame.sprite.collide_circle(bullet, player):
            ...
            draw_health_bar(screen, SCREEN_WIDTH - 320, 20, 300, 15, enemy.hp, enemy.max_hp, RED)
